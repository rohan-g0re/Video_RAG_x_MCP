...
```python
SYSTEM = "You are a helpful assistant ... always cite timestamp and video ID."  # in .jinja
DOC_PROMPT = "[{metadata[start]:.2f}-{metadata[end]:.2f}s | {metadata.video_id}] {page_content}"
QA_CHAIN = RetrievalQA.from_chain_type(
    llm=OpenAI(...),
    retriever=Retriever(...),
    chain_type="stuff",
    chain_type_kwargs={"document_prompt": DOC_PROMPT, "prompt": SYSTEM}
)
```
...
**Deliverables Phase 5**: prompt files, `qa_service.py`, e2e test with canned video and expected timestamp in answer.
